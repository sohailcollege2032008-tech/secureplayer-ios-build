...
## Theme Centralization (pending, as of 2026-07-23)

The accent purple (`0xFF6C63FF`, 133 occurrences across 32 files in the `secure` repo — same pattern likely applies here), background (`0xFF0D0D0D`), and surface (`0xFF1A1A2E`) colors are hardcoded as literals everywhere, no central theme file. User wants a per-brand-controllable `AppColors` class. **Decision made 2026-07-23: implement this on the Mashrou3 branches only for now (`whitelabel-full` + `whitelabel-visual` here, `whitelabel-build` in `secure`), not on the base branches** — user prioritized speed over the cleaner "every future brand inherits it" architecture, given active shipping pressure. Not yet implemented as of this writing — check git log on these branches before assuming it's done.
