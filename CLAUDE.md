...
## ⚠️ Read `SESSION_HANDOFF.md` first

A detailed, dated handoff doc (`SESSION_HANDOFF.md`, repo root, on `whitelabel-full`) covers everything from the 2026-07-23 session in full: what shipped, what's verified working, and — critically — **an unresolved bug where the App Store reviewer demo account's course shows enrolled but the lecture stays permanently locked**. Read that file before doing any further work on the demo-import flow, the App Store submission, or anything else touched that session.
