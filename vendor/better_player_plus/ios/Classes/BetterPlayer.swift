...
public class BetterPlayer: NSObject, FlutterPlatformView, FlutterStreamHandler, AVPictureInPictureControllerDelegate {
    ...
    public func setDataSourceAsset(_ assetPath: String, key: String?, certificateUrl: String?, licenseUrl: String?, cacheKey: String?, cacheManager: CacheManager, overriddenDuration: Int) {
        if let path = Bundle.main.path(forResource: assetPath, ofType: nil) {
            ...
            setDataSourceURL(url, key: key, certificateUrl: certificateUrl, licenseUrl: licenseUrl, offlineFairplayConfig: nil, headers: [:], useCache: false, cacheKey: cacheKey, cacheManager: cacheManager, overriddenDuration: overriddenDuration, videoExtension: nil)
        }
    }

    public func setDataSourceURL(_ url: URL, key: String?, certificateUrl: String?, licenseUrl: String?, offlineFairplayConfig: String?, headers: [AnyHashable: Any], useCache: Bool, cacheKey: String?, cacheManager: CacheManager, overriddenDuration: Int, videoExtension: String?) {
        ...
        if useCache {
            ...
            if let offlineFairplayConfigJson = offlineFairplayConfig, !offlineFairplayConfigJson.isEmpty,
               let certificateUrl = certificateUrl, !certificateUrl.isEmpty {
                ...
                if let certURL = URL(string: certificateUrl),
                   ...
                   let config = try? JSONSerialization.jsonObject(with: configData) as? [String: String] {
                    FairplayContentKeyManager.shared.contentKeyDelegate.configure(
